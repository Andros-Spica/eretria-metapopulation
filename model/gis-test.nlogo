...
globals
[
  patchXpixelScale width height
  elevationData resampledData patchesWithElevationData maxElevation minElevation
  patchArea
]
...
to setup2
  ; Load the elevation raster (in degrees)
  set elevationData gis:load-dataset "gis/Eretria_CHELSA_TraCE21k_dem_-30_V1.asc"

  ; Get the envelope: [min-x min-y max-x max-y] in degrees
  let env gis:envelope-of elevationData
...
print env
  ...
  set width gis:width-of elevationData     ; number of columns
  set height gis:height-of elevationData   ; number of rows
  ...
  set patchArea (R ^ 2) * abs dlon * abs dlat * cos(lat0 * pi / 180) * (pi / 180) ^ 2
...
to setup
  ...
  set patchXpixelScale 12
  set width ceiling ((gis:width-of elevationData) * patchXpixelScale)
  set height ceiling ((gis:height-of elevationData) * patchXpixelScale) - 1

  resize-world 0 width 0 height

  set-patch-size 5 / patchXpixelScale

  ; 2. Match NetLogo world to the dataset envelope
  gis:set-world-envelope gis:envelope-of elevationData

  ; 3. Resample the raster to match world size
  set resampledData gis:resample elevationData gis:world-envelope world-width world-height

  ; 4. Apply the resampled raster to a patch variable
  gis:apply-raster resampledData elevation
  ...
  set minElevation 0;min [elevation] of patchesWithElevationData
